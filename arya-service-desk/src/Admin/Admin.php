...
use Arya\ServiceDesk\Loader;
use Arya\ServiceDesk\Admin\Settings\General;
use Arya\ServiceDesk\Admin\Settings\About;
...
class Admin
{
    ...
    public function plugins()
    {
        add_filter( 'plugin_action_links', function( $actions, $plugin_file, $plugin_data, $context ) {
            ...
            $new_actions['settings'] = sprintf( '<a href="%s">%s</a>', esc_url( $settings ), esc_html__( 'Settings', 'arya-service-desk' ) );

            $about = add_query_arg( [
                'page' => sanitize_key( sprintf( '%s-about', $this->plugin_slug ) )
            ], network_admin_url( 'admin.php' ) );

            $new_actions['about'] = sprintf( '<a href="%s">%s</a>', esc_url( $about ), esc_html__( 'About', 'arya-service-desk' ) );
        ...
        add_filter( 'plugin_row_meta', function( $plugin_meta, $plugin_file, $plugin_data, $status ) {
            ...
            $new_meta = [
                sprintf( '<a href="%s">%s</a>', esc_url( 'https://example.com/' ), esc_html__( 'Documentation', 'arya-service-desk' ) ),
                sprintf( '<a href="%s">%s</a>', esc_url( 'https://example.com/wiki/' ), esc_html__( 'Wiki', 'arya-service-desk' ) )
            ];
